...
test("admin force-delete erases immediately; retention_purge stamps health", async () => {
  ...
  // Unique marker so the assertion + cleanup target ONLY this test's row
  // (never a blanket null-user delete that would wipe other tests' anon consents).
  const marker = `sec-erase-${stamp}`;
  await db().from("consent_log").insert({ user_id: userId, event_type: "terms_accepted", policy_version: marker, metadata: { ip: "1.2.3.4" } } as never);
  ...
  const { data: c } = await db().from("consent_log").select("user_id, metadata").eq("policy_version", marker).single();
  ...
  await db().from("consent_log").delete().eq("policy_version", marker);
